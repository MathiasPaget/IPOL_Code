...
short inf_shr = std::numeric_limits<short>::max()/2;
...
Match::Match(GeneralImage left, GeneralImage right, bool color) {
    ...
    Solution = new short[imSizeL.x*imSizeL.y];
...
void Match::WriteSolution(char *name_file){
	unsigned char *temp=new unsigned char[imSizeL.x*imSizeL.y];
	for(int i=0; i<imSizeL.x*imSizeL.y; i++) temp[i]=(unsigned char)(Solution[i]);
	SaveCimage(temp,imSizeL.x,imSizeL.y,name_file);
	delete[] temp;
};

void Match::Run_Ishikawa(){
	clock_t t2=-clock();
	
	Energy_Ish E = Energy_Ish(imSizeL.x,imSizeL.y,dimg);
	
	short *vec= new short[dimg];
	
	// Fill Graph
	
	//// Fill Data
	for(int i=0; i< imSizeL.x; i++){
		for(int j=0; j<imSizeL.y; j++){
			Coord p=Coord(i,j);
			for(int k=0; k<dimg; k++){
				int d= dispMin + k;
				d= (d <= i ? ( i-d < imSizeL.x ? d : i-imSizeL.x+1)  : i);
				vec[k]=this->data_penalty_color2(p,p-d);
			}
			E.Set_DataCost(i+imSizeL.x*j,vec);
		}
	}	
	
	// Fill Reg
	if( params.lambda1 == params.lambda2) E.Set_UniformL1(params.lambda1);
	else{
		for(int i=0; i< imSizeL.x; i++){
			for(int j=0; j<imSizeL.y; j++){
				int index = i+imSizeL.x*j;
				if(i!=0) E.Set_L1_Reg(index-1,index,this->smoothness_penalty_color(Coord(i,j),Coord(i-1,j)));
				if(j!=0) E.Set_L1_Reg(index-imSizeL.x,index,this->smoothness_penalty_color(Coord(i,j),Coord(i,j-1)));
			}
		}
	}
	...
	// Graph cut
	clock_t t=-clock();
	std::cout << "Global minimum energy " << E.maxflow() << std::endl;
	t+=clock();
	std::cout << "Graph Cut execution time : " << (t/1000) << std::endl;
	
	// Extract solution (node without affectation after optimization, to SOURCE)
	// i.e global minimum with lowest label values
	for(int i=0; i<imSizeL.x*imSizeL.y; i++){
		int count=0;
		for(int k=0; k<dimg-1; k++){
			if( E.what_segment(i*(dimg-1)+k) ==0) count++;
		}
		Solution[i]=count;	
	}
	
	t2+=clock();
	std::cout << "Ish execution time : " << (t2/1000) << std::endl;
}
...
/// Truncked color distance between pixels p and q
int Match::data_penalty_color2(Coord p, Coord q) const {
    int dSum=0;
    // Loop over the 3 channels
    for(int i=0; i<3; i++) {
        int Ip    = IMREF(imColorLeft,     p).c[i];
        int Iq    = IMREF(imColorRight,    q).c[i];
        int d = ( Ip > Iq ? Ip-Iq : Iq-Ip);
        if(d>CUTOFF) d = CUTOFF;
        if(params.dataCost==Parameters::L2) d = d*d;
        dSum += d;
    }
...
/// Smoothness penalty between assignments (p1,p1+disp) and (p2,p2+disp).
int Match::smoothness_penalty_color(Coord p1, Coord p2) const {
    int d, dMax=0; // Max inf norm in RGB space of (p1,p2) and (p1+disp,p2+disp)
    for(int i=0; i<3; i++) {
        d = IMREF(imColorLeft,  p1     ).c[i]-IMREF(imColorLeft,  p2     ).c[i];
        if(d<0) d = -d; if (dMax<d) dMax = d;
